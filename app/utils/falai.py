...
async def _chat_completion(
    ...
    try:
        ...
        if replied_message_id is not None:
            response = await MongoDB.db.completions.find_one(
                {"chat_id": chat_id, "message_id": replied_message_id}
            )
...
Final prompt:\n"""
        ...
        response_text = response_text.replace(
            "Taganrog", "Taganrog a city with a harbor"
        )
        response_text = response_text.replace(
            "taganrog", "Taganrog a city with a harbor"
        )
        response_text = response_text.replace(
            "Таганрог", "Taganrog a city with a harbor"
        )
        response_text = response_text.replace(
            "таганрог", "Taganrog a city with a harbor"
        )
        ...
        result = await httpx_client_no_proxy.post(
            ...
            headers={
                ...
                "Authorization": f"Key {SETTINGS.FAL_AI_API_KEY.get_secret_value().strip()}",
            },
            json={
                ...
                "format": "jpeg",
                "LoraWeight": {"path": loraUrl, "scale": 1},
            },
            timeout=60,
        )
        ...
        image_buf = io.BytesIO(
            base64.b64decode(falai_res[0]["url"].replace("data:image/jpeg;base64,", ""))
        )
        image = BufferedInputFile(file=image_buf.getvalue(), filename="image.jpg")

        newmessage = await answer_message.answer_photo(
            photo=image, caption=response_text, parse_mode="Markdown"
        )
